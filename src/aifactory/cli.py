...
def main():
    ...
    data=json.loads(args.input.read_text());report=compile_factory(FactoryIntent.from_dict(data["factory_intent"]),TelemetryReplay.from_dict(data["telemetry_replay"]));write_artifacts(report,args.output)
